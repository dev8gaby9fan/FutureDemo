...
data class ExchangeInfo(var name: String, var id: String, var sortKey: Int) : Parcelable {
    ...
    fun addInstrument(ins: InstrumentInfo, tradingTime: TradingTime?) {
        ...
        var product = getProduct(ins.productID)
        if (product == null) {
            product = ProductInfo(ins.instrumentName, ins.productID, ins.exchangeID)
            productMap[product.id] = product
        }
        product.tradingTime = tradingTime
    }
    ...
    fun searchInstrument(key: String): List<InstrumentInfo> {
        return getInstruments().filter {
            (it.py?.contains(key, true) ?: false)
                    || it.instrumentName.contains(key, true)
                    || it.productID.contains(key, false)
        }
    }
}
...
data class InstrumentInfo(
    ...
    //合约名称:
    lateinit var instrumentName: String

    //合约ID ExchangeID.InstrumentID
    lateinit var instrumentID: String

    //交易所ID
    lateinit var exchangeID: String
    ...
    //快期JSON字符串中的ins_id
    lateinit var insId: String

    //一跳的价格
    lateinit var priceTick: String
    lateinit var priceDecs: String
